...
class obf:
	...
	def obfuscate(self,file=None,NOTC = "\n\n\t\tOBFUSCATE By MR.GAMING | ASEP RAMDAN SAPUTRA\n\t\tID : %s\n\t\tKEY : %s\n\n",out = ["",False]):
		fov = marshal.dumps(compile(open(file).read(),"Asep_Ramdan","exec"))
		MAC = self.slice(fov)
		if out[1]:
			f = open(out[0],"w")
		else:
		...
		f.write(BLACK_MAGIC+marshal.dumps(compile(MAC,"Asep_Ramdan","exec"))+NOTC%(id,key))
		f.close()
...
zasu = argparse.ArgumentParser(usage="Encrypt -i [ filename ] -o [ outfile ]",description="Obfuscate python2")
...
# cara pake ? Py zasu.py -i file -o out
